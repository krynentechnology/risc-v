 ...
 *
 *  Description: Binary to .INIT parameter conversion to initialize block RAM.
...
int main( int argc, char** argv )
{
    ...
    if ( error ) {
        printf( "usage:    bin2init [option] bin_file\n" );
        ...
        char initFileName[maxFileNameLength+1]; // + '/0' string delimeter!
        int fileNameIndex = 0;
        int dotIndex = -1;
        char* pFile = ( 2 == argc ) ? argv[1] : argv[2];

        while (( pFile[fileNameIndex] != 0 ) && ( fileNameIndex < maxFileNameLength )) {
            initFileName[fileNameIndex] = pFile[fileNameIndex];

            if ( '.' == initFileName[fileNameIndex] ) dotIndex = fileNameIndex;
        ...
        if ( error ) {
            ...
            if ( binFile ) {
                initFileName[++dotIndex] = 'i';
                initFileName[++dotIndex] = 'n';
                initFileName[++dotIndex] = 'i';
                initFileName[++dotIndex] = 't';
                initFileName[++dotIndex] = 0; // String delimeter

                initFile = fopen( initFileName, "wb" );

                if ( NULL == initFile ) printf( "Could not create init file %s\n", initFileName );
            ...
            if ( binFile && initFile ) {
                ...
                char initString[14+64+2+1]; // ".INIT_xx(256'h" ... "),\n"
                int  initIndex = 0;
                char hexNibble;
                char* pInitChar = &initString[77]; // ".)"
                int i;
                ...
                while (( 0 != nbItems ) && ( initIndex < 0x40 )) {
                    ...
                    if ( writeHex ) {
                        ...
                        for ( i = ( size - 1 ); i >= 0; i-- ) {
                            ...
                            *pInitChar = ( hexNibble > 9 ) ? hexNibble + '7' : hexNibble + '0';
                            pInitChar--;
                    ...
                    if ( pInitChar < &initString[14] ) {
                        hexNibble = initIndex & 0x0F; // hexNibble: 15 + '7' = 'F'
                        initString[7] = ( hexNibble > 9 ) ? hexNibble + '7' : hexNibble + '0';
                        hexNibble = ( initIndex >> 4 ) & 0x0F; // hexNibble: 10 + '7' = 'A'
                        initString[6] = ( hexNibble > 9 ) ? hexNibble + '7' : hexNibble + '0';
                        fwrite( initString, sizeof( initString ), 1, initFile );
                        pInitChar = &initString[77];
                        initIndex++;
                    }
                }

                while ((( pInitChar > &initString[13] ) && ( pInitChar < &initString[77] )) || ( initIndex < 0x40 )) {

                    while ( pInitChar > &initString[13] ) {
                        *pInitChar = '0';
                        pInitChar--;
                    }
